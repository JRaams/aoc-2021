...
async function loadDay(day: string): Promise<CalendarDay> {
  const dayFile = await import(`./calendar/${day}`);
  return new dayFile.Day();
}

async function main() {
  const args = loadArgs();
  const day = await loadDay(args.day);

  console.info(`Solving day ${args.day} part ${args.part}...`);
